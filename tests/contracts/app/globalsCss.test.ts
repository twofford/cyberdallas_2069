...
describe('globals.css', () => {
  ...
  it('styles checkboxes with custom terminal controls (not native browser checkboxes)', () => {
    const css = readGlobalsCss();

    expect(css).toContain("body[data-ui='cyberdallas'] input[type='checkbox']");
    expect(css).toContain("body[data-ui='cyberdallas'] input[type='checkbox']::before");
    expect(css).toContain("body[data-ui='cyberdallas'] label[data-checkbox='true']");
  });

  it('styles textarea controls to match terminal input styling', () => {
    const css = readGlobalsCss();

    expect(css).toContain("body[data-ui='cyberdallas'] textarea");
    expect(css).toContain("body[data-ui='cyberdallas'] textarea:focus");
  });
});
